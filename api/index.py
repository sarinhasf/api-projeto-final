...
def gerenciar_filmes():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return (
                jsonify({"success": True, "message": "Filme cadastrado com sucesso!"}),
                201,
            )
...
def deletar_filme(id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Filme removido com sucesso!"})
...
def dashboard():
    ...
    try:
        ...
        brasileiros = sum(1 for f in filmes if f["brasileiro"])

        mais_99 = sum(
            1
            for f in filmes
            if int(str(f.get("aprovacao", "0")).replace("%", "")) >= 99
        )

        antes_2000 = sum(1 for f in filmes if int(f.get("ano", 0)) < 2000)
        ...
        for filme in filmes:
            ...
            if diretor:
                diretores[diretor] = diretores.get(diretor, 0) + 1

        diretor_top = "N/A"
        qtd_diretor = 0

        if diretores:
            diretor_top = max(diretores, key=diretores.get)
